...
# 问题解决
   - 1、浏览http://你到域名或IP:端口，出现白屏的问题。  
     ...
     解决方法：第一步：使用 ` docker inspect [容器名] | jq '.[0].Mounts' ` 查看挂载data的路径，然后把data的权限修改为所有用户都可以写入，或者命令 `sudo chmod 777 [data路径]` ；第二步： 把你当前用户加到docker用户组里，` sudo usermod -a -G docker 用户名`  或者  `sudo usermod -a -G docker $USER `  
     如：
     ``` bash
